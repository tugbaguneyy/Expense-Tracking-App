...
fun RegisterScreen(navController: NavController){
    ...
    Column(modifier = Modifier.fillMaxSize().padding(15.dp),
        ...
        Button(
            modifier = Modifier.fillMaxWidth(),
            onClick = {
                if(mail.value.isNotEmpty() && password.value.isNotEmpty()&& passwordConfirmation.value.isNotEmpty()){
                    viewModel.signUp(mail.value.trim(), password.value.trim(), passwordConfirmation.value.trim())
                }
            }) {
            Text(text = "Register")
        }
